...
// coords for the topmost tile (used in iso_coords()) - other tile positions calculated from first tile
let x_start;
let y_start;
...
class TileType {
  constructor(name, natural = 0, drain = 0, retain = 0, permeate = 0) {
    this.name = name;
    this.image1 = null; //elevations
    this.image2 = null;
    this.image3 = null;
    this.image4 = null;
    this.nature = natural; // point system scoring the values of "good" tiles in the ecosystem
    this.drainage = drain; //output: amount of water that leaves the tile in one iteration
    this.retention = retain; //capacity: how much total water the tile can hold
    this.permeability = permeate; //throughput: how much water the tile can absorb in one iteration
    //this.growth_factor = 0; //how likely will grass spread to this tile/how fast will trees grow on this tile
  } 
}
...
//create another class called compost
...
// let grid = [
//   [new Tile(stone, 0), new Tile(stone, 0), new Tile(grass, 0), new Tile(grass, 1), new Tile(grass, 1), new Tile(grass, 1), new Tile(grass, 1), new Tile(grass, 1)],
...
function setup() {
  ...
  // graphic = createGraphics(width, height); //graphics buffer

  noCursor(); // Hide the default cursor
}
// Adjust canvas size when the window is resized
function windowResized() {
  console.log("In window resized...");
  console.log("Width: ", width, " Height: ", height);
  ...
  // graphic.resizeCanvas(width, height); // Resize the off-screen graphics buffer
}
